...
const stateClasses: Record<NodeState, string> = {
  base: "size-2.5 sm:size-2.5 opacity-95",
  survivor: "size-3 sm:size-3 shadow-[0_0_16px_rgba(255,255,255,0.12)]",
  finalist: "size-3.5 sm:size-3.5 shadow-[0_0_20px_rgba(255,77,77,0.18)]",
  winner:
    "size-4 sm:size-4 border-[rgba(255,77,77,0.35)] bg-[rgba(255,77,77,0.18)] shadow-[0_0_28px_rgba(255,77,77,0.22)]",
  dimmed: "size-2 sm:size-2 opacity-20 saturate-0",
};
        ...
        <AnimatePresence>
          {showLabel ? (
            <motion.div
              ...
              className={cn(
                "absolute left-1/2 top-full mt-1 w-max max-w-[7.5rem] -translate-x-1/2 rounded-md border px-1.5 py-1 text-center shadow-[0_12px_30px_rgba(0,0,0,0.4)] sm:mt-1.5 sm:max-w-[9rem] sm:rounded-lg sm:px-2 sm:py-1",
                state === "winner"
                  ? "border-[rgba(255,77,77,0.22)] bg-[var(--ov-surface-1)]"
                  : "border-[var(--ov-border)] bg-[var(--ov-surface-0)]",
              )}
            >
              <p className="text-[8px] font-semibold tracking-[0.1em] text-[var(--ov-text)] uppercase sm:text-[9px] sm:tracking-[0.12em]">
                {label}
              </p>
              <p className="mt-0.5 truncate text-[9px] font-medium text-[var(--ov-text)] sm:text-[10px]">
                {name}
              </p>
              {subtitle ? (
