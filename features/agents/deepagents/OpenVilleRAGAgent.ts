...
First start the negotiation using openville-negotiate-run.
Then use openville-negotiate-action to reply, accept, or reject based on the user's requirements.
Continue until the negotiation is completed (either accepted or rejected).
...
3. For each of the top 3 candidates sequentially, spawn the "negotiation-handler" task to negotiate with them. Provide the candidate ID and the job description to the negotiation-handler. Do this one at a time.
4. Once all 3 negotiations are complete, gather their results and use openville-select-winner to select the final winner.
